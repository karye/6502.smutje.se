...
Alla 6502-processorer startar på samma sätt: efter reset läser de sk reset-vektorn på adress `$FFFC` och `$FFFD` för att få reda på var programmet börjar. Det är inbyggt i kisel — ingen mjukvara, ingen konfiguration. Genom att koppla in adressbussen och en reset-signal kan jag se detta hända i seriemonitor.
...
Adressbussen läser jag via Arduinons analoga portar — `PORTF` (`A0–A7`) och `PORTK` (`A8–A15`). Det ger mig 16 bitar i två registerläsningar.
...
När jag öppnar seriemonitor (115200 baud) efter uppladdning ser jag ungefär detta:
...
- Jag kopplar en lysdiod med 220 Ω från `A0` (pin 9) till `GND` — A0 växlar på varannan adress, så dioden blinkar i takt med att adresserna räknas.
...
Processorn kan nu berätta var den vill läsa. I nästa steg får den också svar — jag kopplar in databussen.
