...
Att se en processor arbeta i ultrarapid är som att titta på en myrstack: först bara rörelse, sedan mönstret. Med två knappar får jag pausknappen på hela datorn.
...
Kodmässigt byter jag ut den automatiska `loop()` mot knappläsning med debounce (50 ms). Arduinons `INPUT_PULLUP` gör att jag slipper externa pull-up-motstånd — knappen kopplas direkt mellan pinne och `GND`.

## Nya komponenter

Två tryckknappar och en kopplingstråd för `SYNC` — det är allt. Men knapparna är de första komponenterna som ger mig direkt kontroll över exekveringen: en klockcykel eller en hel instruktion per tryck, tack vare att W65C02S är statisk CMOS.
| Antal | Komponent |
|---|---|
| 2 | Tryckknappar |
...
I steg 3 körde CPU:n fritt i en loop — spännande men omöjligt att felsöka. Nu ger jag mig själv full kontroll över exekveringen med två fysiska knappar. Det här är en av de stora fördelarna med W65C02S: eftersom kretsen är statisk CMOS kan klockan stoppas hur länge som helst — minuter, timmar, dagar — utan att processorn tappar sitt interna tillstånd. Jag kan stega genom programmet i min egen takt.
...
- Knapp 1 (`D11`) — klocksteg: varje tryckning ger exakt en klockcykel. Jag ser en ny rad i seriemonitor för varje tryck. Perfekt för att inspektera bussen i ultrarapid.
...
Känslan av att trycka på en knapp och se CPU:n ta ett enda steg är svår att beskriva. Jag har full kontroll. Jag kan stanna hur länge jag vill, mäta spänningar, fundera på vad som händer — och sedan trycka igen. Det är fortfarande mitt favoritsätt att felsöka.

### Så här provar jag

- Jag stegar genom reset-sekvensen med Knapp 1 och räknar klockcyklerna — jag jämför med Knapp 2, som stannar vid varje ny opcode (`SYNC`).
...
Nu kan jag stega genom vilket program som helst. I nästa steg får datorn ett eget ansikte — en LCD-display, så jag slipper laptopen.
