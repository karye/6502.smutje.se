...
Det är en speciell känsla första gången en dator du byggt själv kör ett program du skrivit själv. Nio bytes räcker för att få den känslan.
...
- Låt räknaren börja på 5: ändra `LDX #$00` till `LDX #$05` i `step6.inc` och ladda om.
- Byt `INX` mot `DEX` och se räknaren gå baklänges — den wrappar till 255 efter noll.

## Om det inte fungerar

Här är några saker att kontrollera:

- CPU:n kör `NOP` istället? Du har glömt att ladda det nya programmet. Kontrollera `write_mem()`-anropen i `setup()`.
- CPU:n hoppar till fel adress? `JMP`-adressen är fel. `$8002` är `$02 $80` (låg byte först!).
- Programmet stannar? Du kanske har en `BRK`-instruktion (`0x00`) någonstans. Dubbelkolla att alla bytes är korrekta.

## Vad händer härnäst?

Nu har du skrivit maskinkod som datorn verkligen kör. I nästa steg tar processorn över LCD:n helt själv — vi kopplar in en VIA.
