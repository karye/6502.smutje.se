...
W65C02S är en *statisk CMOS-krets*. Det betyder att klockan kan vara hur långsam som helst — till och med stoppas helt — utan att processorn tappar sitt interna tillstånd. Jag utnyttjar detta genom att generera en långsam klockpuls från Arduino och visualisera den med en lysdiod.
...
- `loop()` — körs *om och om igen i all oändlighet*. När Arduinon nått slutet av `loop()` börjar den om från början — tusentals gånger i sekunden om jag inte saktar ner den. Varje varv i loopen är en chans att läsa sensorer, uppdatera utgångar eller — som i mitt fall — generera en klockpuls.
...
1. Hjälpfunktionen `pulse()` som genererar en komplett klockcykel. Funktionen drar `PHI2` HÖG i 500 ms (lysdioden lyser), sedan LÅG i 500 ms (lysdioden släcks). Resultatet är en symmetrisk fyrkantsvåg på 1 Hz — tillräckligt långsamt för att jag ska kunna följa med i vad som händer. W65C02S är en statisk CMOS-krets, vilket betyder att den inte har någon minsta klockfrekvens — jag kan göra pulserna hur långsamma jag vill utan att processorn tappar sitt interna tillstånd.
1. `setup()` och `loop()` — i `setup()` konfigurerar jag pinnar, håller CPU:n i reset och startar seriekommunikation. I `loop()` anropar jag bara `pulse()` — en klockcykel per varv, för alltid.

### Vad jag ser när koden kör

När jag laddat upp koden och öppnar seriemonitor (115200 baud) ser jag texten "Steg 1 — Klocka och ström". Lysdioden på klocklinjen blinkar en gång per sekund — den är tänd när `PHI2` är hög (CPU:n arbetar) och släckt när `PHI2` är låg. Jag har just gett processorn dess första hjärtslag — en liten, personlig seger.
> [!NOTE] 📦 Arduino-kod — step1.inc · 44 rader · se step1.html

## Exempel på körning

När jag öppnar seriemonitor i Arduino IDE eller PlatformIO (115200 baud) ser jag detta:
...
Det är allt! Programmet har inget mer att rapportera — det bara loopar. Men jag tittar på kopplingsdäcket: lysdioden blinkar i precis den takt som `pulse()` bestämmer. Varje gång dioden tänds är `PHI2` hög och processorn tar ett steg framåt. Varje gång den släcks förbereder processorn nästa steg.
...
Nu har processorn en puls. I nästa steg kopplar jag in adressbussen — då börjar den berätta var den vill titta.
